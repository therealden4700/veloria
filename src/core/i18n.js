 ...
 *
 * Ключи словаря русские, и первое время русским был и язык по умолчанию — просто
 * потому, что так писался код. Но игра лежит в открытом доступе по ссылке, и
 * человек, попавший на неё, скорее прочтёт английский. Русский никуда не делся:
 * он в двух щелчках, в настройках, и выбор запоминается.
 *
 * Выбранный однажды язык сильнее умолчания: `initLang` читает его из настроек и
 * ставит до первой отрисовки.
 */
let lang = 'en';
...
/** Отметить язык в самой странице: по нему работают чтение с экрана и перевод. */
function пометитьСтраницу() {
...
/**
 * Восстановить выбранный язык — до первого нарисованного слова.
 *
 * Раньше это делал конструктор `Game`, а он создаётся уже после экрана
 * загрузки: строки «высекаем камень и дерево» и «разворачиваем карту…» успевали
 * показаться на языке по умолчанию, каким бы он ни был. Первое, что видит
 * человек, не должно быть на языке, который он однажды уже сменил.
 */
export function initLang(сохранённый) {
  if (сохранённый === 'ru' || сохранённый === 'en') lang = сохранённый;
  пометитьСтраницу();
  return lang;
}
