...
/**
 * Иконка лежащей вещи — из долгого кэша, а не с объекта добычи.
 *
 * Кэш стоял на самой записи добычи (`l._и`), и в одиночной игре этого хватало:
 * запись живёт, пока вещь лежит. А в общем мире список добычи целиком
 * заменяется разобранным снимком двадцать раз в секунду — переживать кэшу
 * нечего. Замер: двенадцать вещей на земле, десять кадров, сто двадцать
 * запечённых холстов — ровно по одному на вещь на кадр, и каждый выгружает
 * пиксели из видеопамяти.
 *
 * До поднятия клиенту известны только вид и редкость, а их сочетаний десятки —
 * столько кэш и держит.
 */
const ИКОНКИ_ЛЕЖАЩИХ = new Map();
function иконкаЛежащей(kind, rarity) {
  const ключ = kind + ':' + rarity;
  let и = ИКОНКИ_ЛЕЖАЩИХ.get(ключ);
  if (!и) {
    и = { kind, rarity, icon: itemIcon(kind, null, 0, rarity) };
    ИКОНКИ_ЛЕЖАЩИХ.set(ключ, и);
  }
  return и;
}
...
export class Game {
  ...
  draw() {
    ...
    if (input.набор) {
      // Размеры берём из раскладки, а не из буфера.
      //
      // Слой интерфейса рисуется с масштабом: 480x270 единиц на буфер до
      // 1920x1080. Взять `u.canvas.width` и рисовать этими числами по такому
      // слою — значит уехать вчетверо за край. Замер: полоса строки оказалась
      // на y=966 при экране высотой 270, то есть печатали вслепую.
  ...
  drawLoot(g, l, camX, camY) {
    ...
    if (l.i !== undefined && l.k !== undefined) {
      l = {
        ...
        item: l.k ? иконкаЛежащей(l.k, l.r || 'common') : null,
