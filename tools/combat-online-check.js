...
(async () => {
  ...
  //
  // Полей теперь два. `by` — кто добил, `w` — все, кому комната засчитала
  // участие; клиент считает убийство своим по второму. Пропажа `w` не видна
  // никак: одиночный бот всё равно и добивает, и участвует, — поэтому сторожим
  // её отдельно, иначе правило совместного боя тихо вернётся к «последний удар
  // забирает всё».
  ...
  if (убийства.length) {
    ...
    const сУчастием = убийства.filter((e) => Array.isArray(e.w)).length;
    const вСписке = убийства.filter((e) => Array.isArray(e.w) && e.w.includes(c.pid)).length;
    log(`из них со списком участников ${сУчастием}, мы в списке ${вСписке}`);
    if (сУчастием < убийства.length) {
      note('в событии смерти нет списка участников — помогавший останется без награды');
    } else if (вСписке < наши) {
      note('нас не назвали участником собственного убийства');
    }
  }
