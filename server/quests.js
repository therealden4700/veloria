...
function какИгра(world, ent, вести) {
  ...
  // `spawnLoot` перенаправляем на настоящую комнату — и вот почему.
  //
  // `Object.create` даёт чтение по прототипу, но запись — своему объекту. А
  // `spawnLoot` делает `this.nextLid++`: читает счётчик комнаты, а увеличенное
  // кладёт в собственное поле подставки, которая тут же и умирает. Счётчик
  // комнаты не двигался, и две награды подряд получали один и тот же номер —
  // вторую после этого не поднять, `pickup` находит по номеру первую.
  //
  // Заодно называем хозяина явно: без этого вещь доставалась `world.player`, а
  // это поле означает «на кого сейчас смотрит ИИ», а не «кто сдал задание».
  g.spawnLoot = (x, y, data) => world.spawnLoot(x, y, data, ent);
  return g;
}
