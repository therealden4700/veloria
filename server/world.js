...
export class World {
  ...
  pickup(pid, lid) {
    ...
    this.dirty = true;
    // Помечаем ХОЗЯИНА, а не всю комнату.
    //
    // Общий признак значил, что чужой подбор заставляет каждого в комнате
    // пересобрать весь свой рюкзак — а клиент на этом пересоздаёт иконку каждой
    // вещи. При полусотне игроков один поднятый гриб перестраивал полсотни
    // рюкзаков.
    (this.рюкзакиМеняли ||= new Set()).add(pid);
